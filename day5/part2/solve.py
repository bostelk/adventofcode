...
def line_slope(p0, p1):
    ...
    if dy == 0:
        ...
        return dx / float(dy)
def line_diagonal(p0, p1):
    slope = line_slope(p0,p1)
    return slope.is_integer() if slope != 0 else False
...
def from_to_points(p0, p1):
    ...
    while point != p1:
        ...
        elif line_diagonal(point, p1):
            delta_x = 1 if p1[0] > point[0] else -1
            delta_y = 1 if p1[1] > point[1] else -1
...
with open(filename, 'r') as file:
    ...
    while line:
        ...
        if p1 or p2:
            if line_horizontal(p1, p2) or line_vertical(p1, p2) or line_diagonal(p1, p2):
